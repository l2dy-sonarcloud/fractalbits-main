...
{warp_client_ips}
  ...
  params:
    ...
    # Do RANGE get operations. Will request with random offset and length.
    range: false

    # Use a fixed range size while doing random range offsets.
    # Sizes can be '1KB', '2MB', etc. 'range' is implied.
    range-size:
...
"##
    ...
    run_cmd! {
        mkdir -p $ETC_PATH;
        echo $config_content > $ETC_PATH/bench_get.yml;
        echo $config_content > $ETC_PATH/bench_get.yml.orig;
    }?;
    Ok(())
}
