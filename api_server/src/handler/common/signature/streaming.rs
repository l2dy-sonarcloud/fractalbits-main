        ...
        ContentSha256Header::StreamingPayload { signed, trailer } => {
            ...
            if !signed && !trailer {
                return Err(Error::Other(
                    "STREAMING-UNSIGNED-PAYLOAD without trailer is not a valid combination".into(),
                ));
            }
            ...
            let trailer_algorithm = if trailer {
                let algo = Some(
                    request_trailer_checksum_algorithm(req.headers())?
                        .ok_or_else(|| Error::Other("Missing x-amz-trailer header".into()))?,
                );
                checksummer = checksummer.add_algo(algo);
                algo
            } else {
                None
            };
            ...
            let sign_params = if signed {
                let signature = checked_signature
                    .signature_header
                    .clone()
                    .ok_or_else(|| Error::Other("No signature provided".into()))?;
                let signature = hex::decode(signature)
                    .ok()
                    .and_then(|bytes| Hash::try_from(&bytes))
                    .ok_or_else(|| Error::Other("Invalid signature".into()))?;

                let secret_key = match checked_signature.key {
                    None => return Err(Error::Other("Missing api key".into())),
                    Some(ref key) => key.data.secret_key.clone(),
                };
                ...
                let date: NaiveDateTime = NaiveDateTime::parse_from_str(date, LONG_DATETIME)
                    .map_err(|_e| Error::Other("Invalid date".into()))?;
                let date: DateTime<Utc> = Utc.from_utc_datetime(&date);

                let scope = compute_scope(&date, region, service);
                let signing_hmac = super::signing_hmac(&date, &secret_key, region)
                    .map_err(|_e| Error::Other("Unable to build signing HMAC".into()))?;
            ...
            Ok(req.map(move |body| {
                let stream = body.into_data_stream().map_err(Error::from);
...
mod payload {
    ...
    use super::super::super::data::Hash;
...
mod tests {
    ...
    async fn test_interrupted_signed_payload_stream() {
        ...
        use super::super::super::data::Hash;
        use super::super::{compute_scope, signing_hmac};
        ...
        let secret_key = "test";
        let region = "test";
        let scope = compute_scope(&datetime, region, "s3");
        let signing_hmac = signing_hmac(&datetime, secret_key, region).unwrap();
